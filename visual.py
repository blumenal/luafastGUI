...
class SimpleSaveLuaManager:
    ...
    def add_profile(self, game_info, save_paths, profile_name=None):
        ...
        profile_id = f"{game_info['appid']}_{int(time.time())}"
...
class SaveLuaWindow(customtkinter.CTkToplevel):
    ...
    def __init__(self, master):
        ...
        self.title("Gerenciador de Saves")
        self.geometry("500x800")
        self.resizable(True, True)

        self.save_lua_manager = SimpleSaveLuaManager(self)
    ...
    def refresh_games(self):
        ...
        self.game_widgets = {}
        self.games = self.save_lua_manager.get_installed_steam_games()
    ...
    def configure_profile(self):
        ...
        print(f"DEBUG: Configurando perfil para {self.selected_game['name']}")
    ...
    def do_backup(self):
        ...
        success, message = self.save_lua_manager.create_backup(self.selected_profile)
    ...
    def do_restore(self):
        ...
        backups = self.save_lua_manager.get_backups(self.selected_profile)
    ...
    def manage_backups(self):
        ...
        manage_window.geometry("400x550")
        ...
        def delete_selected():
            if selected_backup[0]:
                if messagebox.askyesno("Confirmar", f"Excluir backup '{selected_backup[0]}'?"):
                    if self.save_lua_manager.delete_backup(self.selected_profile, selected_backup[0]):
    ...
    def remove_selected_profile(self):
        ...
        profile_name = self.save_lua_manager.profiles[self.selected_profile]['name']
        game_name = self.save_lua_manager.profiles[self.selected_profile]['game_name']
        ...
        if messagebox.askyesno("Confirmar Remoção", 
            ...
            success, message = self.save_lua_manager.remove_profile(self.selected_profile)
            if success:
                messagebox.showinfo("Sucesso", message)
                self.selected_profile = None
                self.load_profiles()
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.savelua_button = customtkinter.CTkButton(
            self.navigation_frame,
            text="Save Games",
            command=lambda: self.savelua_button_event(),
        )
        ...
        self.install_frame.grid_rowconfigure(2, weight=1)
        self.install_frame.grid_rowconfigure(3, weight=0)
        self.install_frame.grid_rowconfigure(4, weight=1)
        ...
        self.results_terminal_frame.grid_columnconfigure(0, weight=2)
        self.results_terminal_frame.grid_columnconfigure(1, weight=3)
        ...
        self.textbox.tag_config("progress_line", foreground="blue")
        ...
        self.remove_frame.grid_columnconfigure(0, weight=1)
        self.remove_frame.grid_rowconfigure(0, weight=0)
        self.remove_frame.grid_rowconfigure(1, weight=0)
        self.remove_frame.grid_rowconfigure(2, weight=1)
        self.remove_frame.grid_rowconfigure(3, weight=0)
    ...
    def savelua_button_event(self):
        save_lua_window = SaveLuaWindow(self)
    ...
    def realizar_verificacoes_iniciais(self):
        ...
        print("[DLL'S] Verificando bibliotecas necessárias...", end="", flush=True)
        try:
            from dll_manager import check_and_install
            status_dll, status_msg = check_and_install()
            ...
            print(" AVISO - Erro na verificação de bibliotecas")
            show_warning_and_continue("Erro na Biblioteca", f"Falha na verificação da biblioteca: {str(e)}")

        print("[DEPENDÊNCIES] Verificando módulos...", end="", flush=True)
        try:
            import requests
            import PIL
            import customtkinter
        ...
        for appid in appids:
            ...
            try:
                for item in backup_folder.iterdir():
                    if item.is_file():
                        if item.suffix == ".lua":
                            destino = stplug_path / item.name
                            shutil.copy2(item, destino)
                            success_count += 1
                        elif item.suffix == ".manifest":
                            destino = depotcache_path / item.name
                            shutil.copy2(item, destino)
                            success_count += 1
    ...
    def on_update_search_keyrelease(self, event):
        ...
        if not search_term:
            for appid, data in self.update_selected_games.items():
                if "widget" in data and data["widget"].winfo_exists():
                    data["widget"].grid()
            return
    ...
    def load_update_games(self):
        ...
        for idx, lua_file in enumerate(lua_files):
            ...
            btn = customtkinter.CTkButton(
                self.update_list_frame,
                text=f"Carregando... (ID: {appid})",
                command=lambda a=appid: self.select_update_game(a),
                anchor="w",
                height=30
            )
            btn.grid(row=idx, column=0, sticky="ew", padx=5, pady=2)
            self.update_selected_games[appid] = {"var": var, "widget": btn, "name": None}

    def load_game_name_for_update(self, appid, idx, var):
        try:
            url = f"https://store.steampowered.com/api/appdetails?appids={appid}"
            response = requests.get(url)
            data = response.json()
        ...
        self.safe_after(0, self.update_game_name_for_update, appid, name, idx, var)

    def update_game_name_for_update(self, appid, name, idx, var):
        if appid in self.update_selected_games:
            self.update_selected_games[appid]["name"] = name
            self.update_selected_games[appid]["widget"].configure(text=f"{name} (ID: {appid})")

    def select_update_game(self, appid):
        if self.update_selected_game_id and self.update_selected_game_id in self.update_selected_games:
            self.update_selected_games[self.update_selected_game_id]["widget"].configure(
                fg_color=["#3B8ED0", "#1F6AA5"]
            )

        self.update_selected_game_id = appid
        self.update_selected_games[appid]["widget"].configure(fg_color="green")

        self.update_textbox.delete("1.0", "end")
        self.update_textbox.insert("end", f"\nJogo selecionado para update: {self.update_selected_games[appid]['name']} (ID: {appid})\n", "info")
        self.update_output_redirect.force_scroll()

    def update_selected_game(self):
        if not self.update_selected_game_id:
            self.update_textbox.insert("end", "\nErro: Nenhum jogo selecionado!\n", "error")
            self.update_output_redirect.force_scroll()
            return

        self.update_textbox.insert("end", f"\nIniciando update do jogo: {self.update_selected_games[self.update_selected_game_id]['name']}\n", "info")
        self.update_output_redirect.force_scroll()

        self.run_in_thread_for_update(self.update_game, self.update_selected_game_id)

    def run_in_thread_for_update(self, target_function, *args):
        def runner():
    ...
    async def update_game(self, appid):
        try:
            success, message = await atualizar_do_bruhhub(appid)

            if success:
                self.update_textbox.insert("end", f"\n✅ Update concluído com sucesso!\n", "success")
                self.update_textbox.insert("end", f"📝 {message}\n", "info")
                self.update_output_redirect.force_scroll()
                self.safe_after(0, self.ask_restart_steam_for_update)
            else:
                self.update_textbox.insert("end", f"\n❌ Falha no update: {message}\n", "error")
                self.update_output_redirect.force_scroll()

        except Exception as e:
            error_msg = f"\nErro no update: {str(e)}\n"
            self.update_textbox.insert("end", error_msg, "error")
            self.update_output_redirect.force_scroll()

    def ask_restart_steam_for_update(self):
        self.update_textbox.insert("end", "\nReiniciar Steam agora? (Sim/Não)\n", "info")
        self.update_output_redirect.force_scroll()

        self.safe_after(0, self.show_restart_buttons_for_update)

    def show_restart_buttons_for_update(self):
        ...
        restart_btn = customtkinter.CTkButton(
            self.update_buttons_frame,
            text="Reiniciar Steam",
            command=self.restart_steam_for_update,
            width=120
        )
    ...
    def restart_steam_for_update(self):
        self.update_output_redirect.write("\nReiniciando Steam...\n")
        encerrar_steam_processos()
        reiniciar_steam_fechar()
        self.update_output_redirect.write("✅ Steam reiniciada!\n\n")
        self.update_output_redirect.force_scroll()
    ...
    def load_installed_games_for_dlc(self):
        ...
        for idx, lua_file in enumerate(lua_files):
            ...
            btn = customtkinter.CTkButton(
                self.dlc_games_frame,
                text=f"Carregando... (ID: {appid})",
                command=lambda a=appid: self.select_dlc_game(a),
                anchor="w",
                height=30
            )
            btn.grid(row=idx, column=0, sticky="ew", padx=5, pady=2)
            self.dlc_selected_games[appid] = {"var": var, "widget": btn, "name": None}

    def load_game_name_for_dlc(self, appid, idx, var):
        try:
            url = f"https://store.steampowered.com/api/appdetails?appids={appid}"
            response = requests.get(url)
            data = response.json()
        ...
        self.safe_after(0, self.update_game_name_for_dlc, appid, name, idx, var)

    def update_game_name_for_dlc(self, appid, name, idx, var):
        if appid in self.dlc_selected_games:
            self.dlc_selected_games[appid]["name"] = name
            self.dlc_selected_games[appid]["widget"].configure(text=f"{name} (ID: {appid})")

    def select_dlc_game(self, appid):
        if self.dlc_selected_game_id and self.dlc_selected_game_id in self.dlc_selected_games:
            self.dlc_selected_games[self.dlc_selected_game_id]["widget"].configure(
                fg_color=["#3B8ED0", "#1F6AA5"]
            )

        self.dlc_selected_game_id = appid
        self.dlc_selected_games[appid]["widget"].configure(fg_color="green")

        self.load_dlcs_for_game(appid)
    ...
    def load_dlcs_for_game(self, appid):
        ...
        try:
            ...
            if data and str(appid) in data and data[str(appid)]['success']:
                ...
                for idx, dlc_id in enumerate(dlcs):
                    threading.Thread(
                        target=self.load_dlc_name,
                        args=(dlc_id, idx),
                        daemon=True
                    ).start()
            ...
            error_label = customtkinter.CTkLabel(
                self.dlc_list_frame,
                text=f"Erro ao carregar DLCs: {str(e)}"
            )
            error_label.grid(row=0, column=0, padx=10, pady=10)

    def load_dlc_name(self, dlc_id, idx):
        try:
            url = f"https://store.steampowered.com/api/appdetails?appids={dlc_id}"
            response = requests.get(url)
            data = response.json()
            name = data.get(str(dlc_id), {}).get("data", {}).get("name", f"DLC {dlc_id}")
        except:
            name = f"DLC {dlc_id}"

        self.safe_after(0, self.update_dlc_name, dlc_id, name, idx)

    def update_dlc_name(self, dlc_id, name, idx):
        var = tk.BooleanVar(value=False)

        chk = customtkinter.CTkCheckBox(
            self.dlc_list_frame,
            text=f"{name} (ID: {dlc_id})",
            variable=var,
            command=lambda d=dlc_id, v=var: self.toggle_dlc(d, v)
        )
        chk.grid(row=idx, column=0, sticky="w", padx=10, pady=5)
        self.dlc_selected[dlc_id] = {"var": var, "selected": False}

    def toggle_dlc(self, dlc_id, var):
        self.dlc_selected[dlc_id]["selected"] = var.get()

    def install_selected_dlcs(self):
        if not self.dlc_selected_game_id:
            messagebox.showinfo("Nenhum jogo selecionado", "Selecione um jogo primeiro")
            return

        selected_dlcs = [dlc_id for dlc_id, data in self.dlc_selected.items() 
                        if data.get("selected", False)]
        ...
        confirm_window = customtkinter.CTkToplevel(self)
        confirm_window.title("Confirmar Instalação")
        confirm_window.geometry("400x200")
        confirm_window.resizable(False, False)
        confirm_window.transient(self)
        confirm_window.grab_set()
        ...
        label = customtkinter.CTkLabel(
            main_frame,
            text=f"Tem certeza que deseja instalar {len(selected_dlcs)} DLCs?",
            font=customtkinter.CTkFont(size=14),
            wraplength=350
        )
        label.pack(pady=(20, 30))
        ...
        confirm_btn = customtkinter.CTkButton(
            button_frame,
            text="Sim",
            command=lambda: [self.run_dlc_installation(selected_dlcs), confirm_window.destroy()],
            width=120,
            fg_color="green"
        )
        confirm_btn.grid(row=0, column=0, padx=10)
    ...
    def run_dlc_installation(self, dlc_ids):
        stplug_path = Path(get_steam_path()) / "config" / "stplug-in"
        stplug_path.mkdir(parents=True, exist_ok=True)
        ...
        for dlc_id in dlc_ids:
            try:
                lua_file = stplug_path / f"{dlc_id}.lua"
                with open(lua_file, "w", encoding="utf-8") as f:
                    f.write(f'addappid("{self.dlc_selected_game_id}")\n')
        ...
        messagebox.showinfo("Instalação Concluída", 
                           f"Instalação de DLCs concluída!\n" 
                           f"DLCs instaladas: {success_count}\n"
                           f"Erros: {error_count}")

        self.safe_after(0, self.show_restart_steam_alert)

    def __del__(self):
        if hasattr(self, 'output_redirect'):
            self.output_redirect.stop()
        if hasattr(self, 'update_output_redirect'):
            self.update_output_redirect.stop()
